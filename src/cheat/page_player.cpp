...
#undef ETB_DRAG
	...
	auto player_box = [&](const s_player_entry& entry, SDK::FVector2D pos)
		{
			...
			if (is_dead)
			{
				...
				if (entry.controller && gvalue::world->AuthorityGameMode &&
					function::button_color_text(" ", pos + SDK::FVector2D(250, 5), SDK::FVector2D(90, 30), L"复活"))
				{
			...
			{
				if (function::button_color_text(" ", pos + SDK::FVector2D(250, 5), SDK::FVector2D(40, 30), L"传送") &&
				...
				if (function::button_color_text(" ", pos + SDK::FVector2D(295, 5), SDK::FVector2D(70, 30), L"传送到我") &&
					gvalue::controller && gvalue::controller->Pawn && pawn)
				{
			...
			if (function::button_color_text(" ", pos + SDK::FVector2D(415, 5), SDK::FVector2D(45, 30), L"踢出"))
			{
	...
	const int total_rows = static_cast<int>(param::player_list.size());
	...
	function::text(SDK::FVector2D(param::size.X + 30, 7), L"玩家列表");

	if (function::button_color_text(" ", SDK::FVector2D(param::size.X + 30, 40), SDK::FVector2D(180, 30), L"将所有人传送到我"))
	{
	...
	// 翻页控件
	if (list_page > 0 && function::button_color_text(" ", SDK::FVector2D(param::size.X + 30, 82), SDK::FVector2D(40, 30), L"上页"))
		list_page--;
	if (list_page < max_page && function::button_color_text(" ", SDK::FVector2D(param::size.X + 450, 82), SDK::FVector2D(40, 30), L"下页"))
		list_page++;
	if (max_page > 0)
	{
		const std::wstring page_str = L"第 " + std::to_wstring(list_page + 1) + L" / " + std::to_wstring(max_page + 1) + L" 页";
		function::text(SDK::FVector2D(param::size.X + 75, 89), page_str.c_str());
	}
